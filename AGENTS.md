...
Before adding logic to any module, verify it belongs there. When in doubt, check `constants.ts` first — the value probably already exists.

### Skills available

ThreeJS skills are in `.opencode/skills/`. Consult before implementing anything Three-related:
...
**Rule:** Read the relevant skill before writing Three.js code. Don't guess API surface.
...
- No non-null assertions (`!`) unless the value is guaranteed and you explain why inline.
...
- Never construct objects inside the render loop (`new THREE.Vector3()` etc.) — allocate outside, reuse inside.
...
- Scene is loaded from `public/scene1.glb` — do not duplicate asset loading logic outside `SceneLoader.ts`.

### File conventions

- One class/concept per file. Filename = class name (PascalCase).
- No barrel files (`index.ts`) unless explicitly requested.
- Shared types: define inline or in the file that owns the concept. No `types.ts` dumping ground.
...
- **No game state in `main.ts`** — it only bootstraps. State lives in Engine or dedicated modules.
...
- **No dynamic imports** unless there's a clear perf reason. Keep the module graph simple.
- **No modifications to `dist/`** — it's generated. Never edit files there.
...
If a new asset is needed: add to `/public/`, load it in the appropriate module, do not scatter loading logic.
...
- Changing the public asset pipeline
...
- Add a test framework without explicit request
